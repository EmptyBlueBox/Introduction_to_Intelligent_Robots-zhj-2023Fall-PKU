...
bool DECOFUNC(processMultiInputData)(void *paramsPtr, void *varsPtr, QVector<QVector<void *>> inputParams, QVector<QVector<void *>> inputData, void *outputData, QList<int> &outputPortIndex)
{
	...
	double P = 0.5, I = 0.2, D = 0.1, error_max = 100, straight_max_steer = 250; // PID参数
	...
	//	// 判断前面是否有锥桶
	//	int cone_ahead_thres = 80, cone_ahead_cnt = 30;
	//	int is_cone = 0, continue_cnt = 0;
	//	for (int i = 0; i <= 360; i++)
	//	{
	//		if (inputdata_1.front()->data[i] < cone_ahead_thres)
	//		{
	//			// 如果上一个激光大于阈值
	//			if (is_cone == 0)
	//			{
	...
	//			{
	//				continue_cnt++;
	//				if (continue_cnt > cone_ahead_cnt)
	//				{
